...
public class YearEvent : MonoBehaviour {
	...
	public bool MostraYearEvent = false;
	private float timer;
	...
	private GameObject YearEventPanel;

	void Start () {
		YearEventPanel = gameObject.transform.GetChild (0).gameObject;
	...
	void Update () {
		if (MostraYearEvent) {
			...
			if (timer <= 0.0f) {
				MostraYearEvent = false;
			}
		} else {
			timer = YearEventDisplayTime;
		}
		YearEventPanel.SetActive (MostraYearEvent);
	}
	...
	void UpdateYear (string value)
	{
		...
		if (currentYear != year) {
			if (yearEventsList.ContainsKey (year)) {
				YearEventText.text = yearEventsList [year];
				EventManager.TriggerEvent ("newDiaryEntry", "<b>"+year+"</b> - " + yearEventsList [year]);
			} else {
				YearEventText.text = "Ano Novo";
			}		
			MostraYearEvent = true;
		}

		YearText.text = currentYear = year;
	}
}
